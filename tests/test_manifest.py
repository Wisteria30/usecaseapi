...
def test_manifest_cli_uses_yaml_for_export_validate_scaffold_docs_graph_and_diff(
    ...
    monkeypatch.syspath_prepend("examples/basic")
    ...
    assert main(["manifest", "check-sync", "app.composition:usecases", str(manifest_path)]) == 0
    ...
    assert (generated_root / "app/contracts/orders/place_order/v1.py").exists()

    assert main(["docs", str(manifest_path), "--output", str(docs_path)]) == 0
    assert "orders.place_order v1" in docs_path.read_text()
...
def test_manifest_cli_covers_error_and_stdout_branches(
    ...
    assert main(["manifest", "export", "app.composition:usecases", "-o", str(manifest_path)]) == 0
    ...
    assert main(["manifest", "check-sync", "app.composition:usecases", str(changed_path)]) == 1
    assert "Breaking:" in capsys.readouterr().out

    assert main(["docs", str(manifest_path), "-o", str(docs_path)]) == 0
    assert docs_path.exists()

    args = argparse.Namespace(manifest_command="unknown")

    with pytest.raises(ValueError, match="unknown manifest command"):
        cli._cmd_manifest(args)
