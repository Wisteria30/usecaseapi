...
The release workflow publishes only on pushed tags matching `v*`.
...
7. Update the release version in `pyproject.toml` and `src/usecaseapi/__init__.py`.
8. Prepare release notes.
9. Push an annotated release tag.
